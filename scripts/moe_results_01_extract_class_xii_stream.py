...
Output: extractions/moe_results_class_xii_stream_2020-2024.csv
...
OUT = ROOT / "extractions" / "moe_results_class_xii_stream_2020-2024.csv"
...
def parse_stream_rows(table, year, category):
    ...
    for r in table:
        ...
        if not is_2020_style:
            ...
            if not sno.isdigit():
                continue
            state = (r[1] or "").strip() if len(r) > 1 else ""
            if state:
                last_state = state
            else:
                state = last_state
            board = (r[2] or "").strip() if len(r) > 2 else ""
            if not board:
                continue
            for idx in range(15):
                dc = 3 + idx
...
def main():
    ...
    for cfg in TABLES:
        year = cfg["year"]
        pdf_path = SOURCES / f"moe_results_secondary_hs_{year}.pdf"
    ...
    with OUT.open("w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["year", "state", "board", "social_category", "stream", "gender", "students_passed"])
        for r in all_rows:
            # Only write rows where value is non-null (drop blacked-out cells)
            year, state, board, cat, stream, gender, val = r
            if val is None:
                continue
            w.writerow([year, state, board, cat, stream, gender, val])
